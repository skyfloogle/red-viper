...
static Result sendAndReceive(const void *request, int request_size, void *response, int response_size, Handle *two_events, int64_t timeout) {
    Result subres, res;
    for (int i = 0; i < 4; i++) {
...
static void iruserThreadFunc(void* param) {
    ...
    while (true) {
        ...
        ret = sendAndReceive(&calibration_request, sizeof(calibration_request), &calibration_response, sizeof(calibration_response), two_events, 20 * MILLIS);
        ...
        while (iruserSharedMemAddr->header.connection_status == 2) {
            ...
            ret = sendAndReceive(&input_request, sizeof(input_request), &input_response, sizeof(input_response), two_events, 20 * MILLIS);
            if (ret == SAR_EXIT) goto cleanup;
