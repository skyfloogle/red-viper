...
bool V810_DSP_Init()
{
	...
	finalScreen[0] = C3D_RenderTargetCreate(240, 400, GPU_RB_RGBA8, GPU_RB_DEPTH16);
	C3D_RenderTargetSetOutput(finalScreen[0], GFX_TOP, GFX_LEFT, DISPLAY_TRANSFER_FLAGS);
	finalScreen[1] = C3D_RenderTargetCreate(240, 400, GPU_RB_RGBA8, GPU_RB_DEPTH16);
...
void sceneRender()
{
	float cols[4] = {0, 3.7 / 6, 4.5 / 6, 1.0};
			...
			else
			{
				...
				else
				{
					...
					for (int y = 0; y < h; y++)
					{
						...
						avcur->x2 = gx + w;
						avcur->y2 = gy + y + 1;
						avcur->u = mx - (eye == 0 ? mp * dx >> 6 : 0);
						avcur->v = my - (eye == 0 ? mp * dy >> 6 : 0);
						avcur->ix = dx * (w + mp) >> 6;
						avcur->iy = dy * (w + mp) >> 6;
